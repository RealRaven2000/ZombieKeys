...
let isComposeLog = false; // will be set from local storage in init() function.
  ...
  function serializeKeyEvent(event) {
    ...
    // let result = await keyPressHandler(serializeKeyEvent(event)); - [ComposeScripts]
    ...
    // let result = await keyUpHandler(serializeKeyEvent(event)); [ComposeScript]
...
*/
  if (isComposeLog) {
    console.log("ZombieKeys - compose init() Registering event listeners...");
  }
